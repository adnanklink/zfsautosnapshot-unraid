(() => {
  ...
  async function refresh() {
    ...
    try {
      const data = await json('/plugins/zfs.autosnapshot/php/coordinator-status.php', {cache: 'no-store', signal: requestController.signal});
  ...
  async function action(name) {
    if (busy) return;
    busy = true; ++generation; if (controller) controller.abort(); clearTimeout(timer);
    ...
    try {
      await json('/plugins/zfs.autosnapshot/php/coordinator-action.php', {method: 'POST', signal: requestController.signal,
