...
{
    try { zfsas_coordinator_request(['action' => 'watchdog'], '/var/run/zfs-autosnapshot-coordinator/control.sock', .2); return; }
    catch (RuntimeException $error) {}
    if (is_file('/boot/config/plugins/zfs.autosnapshot/maintenance')) { throw new RuntimeException('Plugin maintenance is in progress.'); }
    $daemon = __DIR__ . '/coordinator-daemon.php';
    $detach = __DIR__ . '/../scripts/detach-worker.sh';
    exec('nohup /bin/bash ' . escapeshellarg($detach) . ' php ' . escapeshellarg($daemon) . ' >> /var/log/zfs_autosnapshot_coordinator.log 2>&1 < /dev/null &');
