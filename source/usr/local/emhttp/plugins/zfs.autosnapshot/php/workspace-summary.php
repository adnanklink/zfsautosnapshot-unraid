...
function zfsas_workspace_summary(): array
{
    ...
    try {
        $response = zfsas_coordinator_request(['action' => 'status'], '/var/run/zfs-autosnapshot-coordinator/control.sock', .5);
    ...
    try {
        ...
        if ($state !== '' && $state !== 'idle') {
            ...
            $result['operations'][] = ['id' => 'migration:current', 'nativeId' => 'current', 'type' => 'migration',
                ...
                'actions' => [], 'url' => '/Settings/ZFSAutoSnapshot?section=tools&tab=migrator', 'logType' => 'migration'];
        }
