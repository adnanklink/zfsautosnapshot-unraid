...
$debugLogFile = '/var/log/zfs_snapsync.log';
$debugArchiveLogFile = '/var/log/zfs_snapsync.archive.log';
$summaryLogFile = '/var/log/zfs_snapsync.last.log';
...
function downloadCombinedLogs($debugLogFile, $debugArchiveLogFile, $summaryLogFile)
{
    if (!headers_sent()) {
        ...
        header('Content-Disposition: attachment; filename="zfs_snapsync_logs.txt"');
        header('Cache-Control: no-store, no-cache, must-revalidate, max-age=0');
        header('Pragma: no-cache');
        header('X-Content-Type-Options: nosniff');
    }

    echo "ZFS SnapSync Log Export\n";
