...
$downloadName = 'zfs_snapsync_diagnostics.zip';
...
function zfsas_diagnostics_safe_source_path($path, $allowKnownSymlink = false)
{
    ...
    $allowedExactPaths = [
        ...
        '/var/log/plugins/zfs.snapsync.plg',
    ];
    ...
    $allowedRoots = [
        '/var/log',
        '/var/log/plugins',
        '/boot/config/plugins/zfs.snapsync',
...
function zfsas_diagnostics_write_zfs_summary($baseDir)
{
    ...
    foreach ($snapshotLines as $line) {
        ...
        $prefix = 'other';
        if (strpos($snapshot, 'snapsync-auto-') === 0) {
            $prefix = 'autosnapshot';
        } elseif (strpos($snapshot, 'snapsync-send-') === 0) {
            $prefix = 'zfs-send';
        }
...
function zfsas_diagnostics_write_send_summary($baseDir)
{
    ...
    $sendByDataset = [];
    foreach ($snapshotLines as $line) {
        if (strpos($line, '@snapsync-send-') === false) {
            continue;
        }
...
$zipPath = $tempRoot . '/zfs_snapsync_diagnostics.zip';
...
zfsas_diagnostics_write_file($payloadDir, 'README.txt', "ZFS SnapSync diagnostics export\nGenerated: " . gmdate('Y-m-d H:i:s') . " UTC\nSecrets and public topology details are redacted or summarized before packaging.\nAttach this zip to a GitHub issue when reporting plugin problems.\n");
zfsas_diagnostics_write_file($payloadDir, 'metadata/environment.txt', "Generated UTC: " . gmdate('Y-m-d H:i:s') . "\nPHP: " . PHP_VERSION . "\nSAPI: " . PHP_SAPI . "\nPlugin: zfs.snapsync\n");

$files = [
    'logs/zfs_snapsync.last.log' => '/var/log/zfs_snapsync.last.log',
    'config/plugin_manifest.plg' => '/boot/config/plugins/zfs.snapsync.plg',
    'installed/plugin_manifest.plg' => '/var/log/plugins/zfs.snapsync.plg',
];
...
zfsas_diagnostics_write_config_summary($payloadDir, '/boot/config/plugins/zfs.snapsync/zfs_snapsync.conf', '/boot/config/plugins/zfs.snapsync/zfs_send.conf');
zfsas_diagnostics_write_log_summary($payloadDir, 'logs/zfs_snapsync.summary.txt', '/var/log/zfs_snapsync.log');
zfsas_diagnostics_write_log_summary($payloadDir, 'logs/zfs_snapsync.archive.summary.txt', '/var/log/zfs_snapsync.archive.log');
zfsas_diagnostics_write_log_summary($payloadDir, 'logs/zfs_snapsync_send.summary.txt', '/var/log/zfs_snapsync_send.log');
zfsas_diagnostics_write_log_summary($payloadDir, 'logs/zfs_snapsync_send.archive.summary.txt', '/var/log/zfs_snapsync_send.archive.log');
...
$commands = [
    ...
    'commands/plugin-package.txt' => 'ls -1 /var/log/packages/zfs-snapsync-* 2>/dev/null || true',
];
