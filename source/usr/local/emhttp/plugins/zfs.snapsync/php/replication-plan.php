...
function zfsas_replication_plan(array $request, array $inspection, string $revision, string $rateLimit = '0'): array
{
    ...
    if (!in_array($inspection['mode'] ?? '', ['full','incremental','already_received'], true)) {
...
function zfsas_replication_space(array $parameters): array
{
    ...
    $estimate = ZfsasReplicationInspection::command(array_merge(['send','-nP'],$base === null ? [] : ['-i',$base],[$request['sourceSnapshot']]));
    ...
    $capacity = !empty($request['createDestination']) ? substr($request['destination'],0,strrpos($request['destination'],'/')) : $request['destination'];
