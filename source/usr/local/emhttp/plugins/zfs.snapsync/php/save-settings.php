...
$defaultReturnUrl = '/Settings/ZFSSnapSync';
...
set_error_handler(function ($severity, $message, $file, $line) {
    error_log(sprintf(
        'zfs.snapsync save-settings warning: %s in %s:%d',
        (string) $message,
        (string) $file,
        (int) $line
    ));
...
set_exception_handler(function ($throwable) {
    error_log(sprintf(
        'zfs.snapsync save-settings exception: %s in %s:%d',
