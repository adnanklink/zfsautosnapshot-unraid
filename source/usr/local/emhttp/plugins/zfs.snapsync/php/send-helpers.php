...
function zfsas_read_auto_snapshot_prefix($configDir, $defaultPrefix = 'snapsync-auto-')
{
    $path = rtrim((string) $configDir, '/') . '/zfs_snapsync.conf';
    if (!is_file($path)) {
...
function zfsas_send_render_config($config)
{
    $lines = [];
    $lines[] = '# -----------------------------------------------------------------------------';
    $lines[] = '# ZFS send replication config for ZFS SnapSync';
    $lines[] = '# Path: /boot/config/plugins/zfs.snapsync/zfs_send.conf';
    $lines[] = '# -----------------------------------------------------------------------------';
    $lines[] = '';
