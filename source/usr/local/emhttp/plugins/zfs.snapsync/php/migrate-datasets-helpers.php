...
{
    return '/tmp/zfs-snapsync-migrator';
}
...
{
    return '/var/log/zfs_snapsync_migrate_datasets.log';
}

function zfsas_migrate_log_archive_file()
{
    return '/var/log/zfs_snapsync_migrate_datasets.archive.log';
}

function zfsas_migrate_worker_script()
{
    return '/usr/local/sbin/zfs_snapsync_migrate_datasets';
}
...
{
    if (is_file('/boot/config/plugins/zfs.snapsync/maintenance')) { $error = 'Plugin maintenance is in progress.'; return false; }
    $error = null;
