...
$defaultReturnUrl = '/plugins/zfs.snapsync/php/send-settings.php';
...
set_error_handler(function ($severity, $message, $file, $line) {
    error_log(sprintf(
        'zfs.snapsync save-send-settings warning: %s in %s:%d',
        (string) $message,
        (string) $file,
        (int) $line
    ));
...
register_shutdown_function(function () {
    ...
    zfsas_send_standalone_error_page(
        ...
        (string) ($GLOBALS['zfsas_send_save_endpoint_return_url'] ?? '/plugins/zfs.snapsync/php/send-settings.php'),
        500
    );
});
...
set_exception_handler(function ($throwable) {
    error_log(sprintf(
        'zfs.snapsync save-send-settings exception: %s in %s:%d',
