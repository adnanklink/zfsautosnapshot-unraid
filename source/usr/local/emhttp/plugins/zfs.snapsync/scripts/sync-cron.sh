...
CONFIG_FILE="${CONFIG_DIR}/zfs_snapsync.conf"
SEND_CONFIG_FILE="${CONFIG_DIR}/zfs_send.conf"
CRON_FILE="/etc/cron.d/zfs_snapsync"
RUN_CMD="/usr/local/sbin/zfs_snapsync_coordinator watchdog"
QUEUE_KICKER_CMD="/usr/local/sbin/zfs_snapsync_queue_kicker"

if [[ "${ZFSAS_CONFIG_LOCK_HELD:-0}" != 1 ]]; then
  mkdir -p /tmp/zfs-snapsync-config-locks
  config_lock_key="$(printf %s "$CONFIG_DIR" | sha256sum | cut -d ' ' -f1)"
  exec 8>"/tmp/zfs-snapsync-config-locks/${config_lock_key}.lock"
  chmod 0660 "/tmp/zfs-snapsync-config-locks/${config_lock_key}.lock"
  chown nobody:users /tmp/zfs-snapsync-config-locks "/tmp/zfs-snapsync-config-locks/${config_lock_key}.lock" 2>/dev/null || true
  flock 8
fi
...

{
	...
	if [[ -n "$CRON_SCHEDULE_EFFECTIVE" ]]; then
		echo "${CRON_SCHEDULE_EFFECTIVE} ${RUN_CMD} >> /var/log/zfs_snapsync.log 2>&1"
	fi
	echo "* * * * * /usr/local/sbin/zfs_snapsync_coordinator watchdog >> /var/log/zfs_snapsync_coordinator.log 2>&1"
	echo "* * * * * ${QUEUE_KICKER_CMD} >> /var/log/zfs_snapsync_send.log 2>&1"
} >"$CRON_FILE_TMP"
