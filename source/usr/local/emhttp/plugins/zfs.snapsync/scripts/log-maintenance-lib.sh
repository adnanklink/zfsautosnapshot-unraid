...
zfsas_log_dir_is_plugin_owned() {
  ...
  case "$dir" in
    /var/log/zfs_snapsync*|/var/run/zfs-snapsync*|/tmp/zfs-snapsync*|/boot/config/plugins/zfs.snapsync/*)
      return 0
      ;;
  esac
...
zfsas_log_prepare_for_append() {
  ...
  mkdir -p /tmp/zfs-snapsync-log-compact >/dev/null 2>&1 || true
  lock_token="$(zfsas_log_compaction_lock_token "$log_path")"
  lock_dir="/tmp/zfs-snapsync-log-compact/${lock_token}.lockdir"
  lock_file="/tmp/zfs-snapsync-log-compact/${lock_token}.lock"
