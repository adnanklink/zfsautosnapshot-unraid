...
ZFSAS_LOG_HELPER="/usr/local/emhttp/plugins/zfs.snapsync/scripts/log-maintenance-lib.sh"
...
FAILED_SEND_LOGS_DIR="/var/log/zfs-snapsync-failed-sends"
SEND_SCHEDULE_STATE_FILE="${OPS_STATUS_DIR}/send_schedule_state.state"
RUNTIME_DIR="/var/run/zfs-snapsync-ops"
...
AUTOSNAPSHOT_RUNTIME_DIR="/var/run/zfs-snapsync"
AUTOSNAPSHOT_ACTIVE_FILE="${AUTOSNAPSHOT_RUNTIME_DIR}/zfs_snapsync.active"
LOG_FILE="/var/log/zfs_snapsync_send.log"
LOG_ARCHIVE_FILE="/var/log/zfs_snapsync_send.archive.log"
SEND_LOG_MAX_BYTES="${SEND_LOG_MAX_BYTES:-2097152}"
SEND_LOG_ARCHIVE_MAX_BYTES="${SEND_LOG_ARCHIVE_MAX_BYTES:-4194304}"

DEFAULT_SEND_SNAPSHOT_PREFIX="snapsync-send-"
...
start_delete_queue_daemon() {
  ...
  [[ -x /usr/local/sbin/zfs_snapsync_coordinator ]] || return 1
  /usr/local/sbin/zfs_snapsync_coordinator delete >/dev/null
}
...
prefix_configuration_safe() {
  ...
  local auto_prefix=snapsync-auto- line raw
  if [[ -f "$CONFIG_DIR/zfs_snapsync.conf" ]]; then
    while IFS= read -r line; do
      if [[ "$line" =~ ^[[:space:]]*PREFIX[[:space:]]*=(.*)$ ]]; then
        raw="${BASH_REMATCH[1]}"; auto_prefix="$(parse_config_value "$raw")"
      fi
    done < "$CONFIG_DIR/zfs_snapsync.conf"
  fi
