...
mkdir -p /boot/config/plugins/zfs.snapsync
: > /boot/config/plugins/zfs.snapsync/maintenance

PLUGIN_NAME="zfs.snapsync"
PLUGIN_DIR="/usr/local/emhttp/plugins/${PLUGIN_NAME}"
BOOT_PLUGIN_DIR="/boot/config/plugins/${PLUGIN_NAME}"
DEFAULT_CFG="${PLUGIN_DIR}/config/zfs_snapsync.conf.example"
TARGET_CFG="${BOOT_PLUGIN_DIR}/zfs_snapsync.conf"
DEFAULT_SEND_CFG="${PLUGIN_DIR}/config/zfs_send.conf.example"
TARGET_SEND_CFG="${BOOT_PLUGIN_DIR}/zfs_send.conf"
MAIN_TARGET_CFG="${BOOT_PLUGIN_DIR}/zfs_snapsync.conf"
REPAIR_PERMS_SCRIPT="${PLUGIN_DIR}/scripts/repair-permissions.sh"
RUNTIME_DIR="/var/run/zfs-snapsync"
LOCK_FILE="${RUNTIME_DIR}/zfs_snapsync.lock"
LOCK_DIR="${RUNTIME_DIR}/zfs_snapsync.lockdir"
CHILD_PID_FILE="${RUNTIME_DIR}/zfs_snapsync.child.pid"
STOP_FILE="${RUNTIME_DIR}/zfs_snapsync.stop"
RUN_MATCH='/usr/local/sbin/zfs_snapsync'
SNAPSHOT_PREFIX='snapsync-auto-'
SEND_RUNTIME_DIR="/var/run/zfs-snapsync-send"
SEND_LOCK_FILE="${SEND_RUNTIME_DIR}/zfs_snapsync_send.lock"
SEND_LOCK_DIR="${SEND_RUNTIME_DIR}/zfs_snapsync_send.lockdir"
SEND_CHILD_PID_FILE="${SEND_RUNTIME_DIR}/zfs_snapsync_send.child.pid"
SEND_STOP_FILE="${SEND_RUNTIME_DIR}/zfs_snapsync_send.stop"
SEND_RUN_MATCH='/usr/local/sbin/zfs_snapsync_send'
ALLOW_ORPHAN_DESTROY_MATCHES=0
OPS_RUNTIME_DIR="/var/run/zfs-snapsync-ops"
...
OPS_KICKER_RUN_MATCH='/usr/local/sbin/zfs_snapsync_queue_kicker'
OPS_HANDLER_RUN_MATCH='/usr/local/sbin/zfs_snapsync_queue_handler'
OPS_SEND_WORKER_RUN_MATCH='/usr/local/sbin/zfs_snapsync_send_worker'
OPS_DELETE_WORKER_RUN_MATCH='/usr/local/sbin/zfs_snapsync_delete_worker'
OPS_JOB_LOCKS_DIR="${OPS_RUNTIME_DIR}/job-locks"
RECOVERY_RUNTIME_DIR="/var/run/zfs-snapsync-recovery"
...
RECOVERY_SCAN_RUN_MATCH='/usr/local/sbin/zfs_snapsync_recovery_scan'
MIGRATOR_RUNTIME_DIR="/var/run/zfs-snapsync-migrator"
...
MIGRATOR_RUN_MATCH='/usr/local/sbin/zfs_snapsync_migrate_datasets'
SNAPSHOT_MANAGER_RUNTIME_DIR="/var/run/zfs-snapsync-manager"
...
SNAPSHOT_MANAGER_RUN_MATCH='/usr/local/sbin/zfs_snapsync_snapshot_manager_worker'
LEGACY_OPS_QUEUE_DIR="${BOOT_PLUGIN_DIR}/ops_queue"
TMP_OPS_QUEUE_DIR="/tmp/zfs-snapsync-ops"
...
request_graceful_stop() {
  ...
  echo "Requested running zfs_snapsync job to stop."
  sleep 2
}

source /usr/local/emhttp/plugins/zfs.snapsync/scripts/worker-shutdown-lib.sh
...
stop_running_jobs() {
  ...
  if [[ -z "$(printf '%s' "$seen_pids" | sed '/^[[:space:]]*$/d')" ]]; then
    echo "No running zfs_snapsync job detected."
  else
    while IFS= read -r pid; do
      [[ -n "$pid" ]] || continue
      stop_pid_tree "$pid"
...
TARGET_CFG="$TARGET_SEND_CFG"
ALLOW_ORPHAN_DESTROY_MATCHES=0
for RUN_MATCH in "/usr/local/emhttp/plugins/zfs.snapsync/php/coordinator-daemon.php" "/usr/local/emhttp/plugins/zfs.snapsync/scripts/coordinator-attempt.sh" "$OPS_KICKER_RUN_MATCH" "$OPS_HANDLER_RUN_MATCH" "$OPS_SEND_WORKER_RUN_MATCH" "$OPS_DELETE_WORKER_RUN_MATCH" "/usr/local/emhttp/plugins/zfs.snapsync/php/snapshot-batch-worker.php"; do
  stop_running_jobs
done
zfsas_stop_recorded_send_groups
php /usr/local/emhttp/plugins/zfs.snapsync/php/coordinator-shutdown.php
...
rm -f /boot/config/plugins/zfs.snapsync/maintenance
exit 0
