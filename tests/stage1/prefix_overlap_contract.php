...
assert_false(zfsas_snapshot_prefixes_conflict('snapsync-auto-', 'snapsync-send-'), 'distinct default prefixes should be allowed');
assert_true(zfsas_snapshot_prefixes_conflict('snapsync-send-', 'snapsync-send-'), 'identical auto/send prefixes must be rejected');
assert_true(zfsas_snapshot_prefixes_conflict('snapsync-', 'snapsync-send-'), 'auto prefix that can match send checkpoints must be rejected');
assert_true(zfsas_snapshot_prefixes_conflict('snapsync-send-daily-', 'snapsync-send-'), 'auto prefix under the send checkpoint namespace must be rejected');

$message = zfsas_snapshot_prefix_conflict_message('snapsync-', 'snapsync-send-');
assert_contains('Auto-snapshot prefix', $message, 'conflict message should identify the auto-snapshot prefix');
assert_contains('snapsync-', $message, 'conflict message should include the auto prefix');
assert_contains('snapsync-send-', $message, 'conflict message should include the send prefix');
...
file_put_contents($tempDir . '/zfs_snapsync.conf', 'PREFIX="snapsync-"' . PHP_EOL);
$post['config_revision'] = zfsas_config_revision($tempDir);
$result = zfsas_send_handle_save_request($post, $tempDir, $configFile, $syncScript, $config, '/Settings/ZFSSnapSyncSend', 'snapsync-');
