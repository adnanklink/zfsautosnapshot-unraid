...
php -r 'require $argv[1]; zfsas_config_read_pair("/boot/config/plugins/zfs.snapsync");' "$plugin/php/send-helpers.php"
...
(
  source <(sed '/^for cmd in zfs /,$d' "$root/source/usr/local/sbin/zfs_snapsync_migrate_datasets")
...
! grep -q 'load_boot_persisted_state\|write_boot_persisted_snapshot\|PERSIST_QUEUE_ON_EXIT' "$root/source/usr/local/sbin/zfs_snapsync_delete_worker"
