...
if grep -En 'zfs destroy -r|purge_destination_for_reseed|receive.*-uF' "$ROOT/source/usr/local/sbin/zfs_snapsync_send_worker" "$ROOT/source/usr/local/emhttp/plugins/zfs.snapsync/scripts/ops-queue-lib.sh"; then exit 1; fi
