...
$decoder=static function($args)use($read){if($args[0]==='send')return "toname = tank/source@new\ntoguid = 0x12c\nfromguid = 0xc8\n";return $read($args);};
$approved=$request+['allowResume'=>true];
$result=ZfsasReplicationInspection::inspect($approved,$decoder);
check($result['inspection']['mode']==='resume' && count($result['inspection']['references'])===3,'Explicit resume omitted its base references');
check(!str_contains(json_encode($result),'secret-resume-token'),'Resume plan retained raw token');
$wrong=static function($args)use($decoder){return $args[0]==='send'?"toname = tank/source@new\ntoguid = 0x999\n":$decoder($args);};
reject(fn()=>ZfsasReplicationInspection::inspect($approved,$wrong));
$wide=static function($args)use($read){if($args[0]==='send')return "toname = tank/source@new\ntoguid = 0xffffffffffffffff\n";if(end($args)==='tank/source@new')return '18446744073709551615';return $read($args);};
$result=ZfsasReplicationInspection::inspect(array_replace($approved,['sourceGuid'=>'18446744073709551615']),$wide);
check($result['inspection']['mode']==='resume','Unsigned resume GUID lost precision');
